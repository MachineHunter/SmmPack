...
public class GetSmmProtocolInterfaceStructureAddress extends HeadlessScript {
    private ClangNode process_clang_node(ClangNode cn) {
		 ...
		 if(numChildren!=0) {
			 ...
			 for(i=0; i<numChildren; i++) {
				 ...
				 if(String.valueOf(child).contains("SmmInstallProtocolInterface")) {
    ...
    public void run() throws Exception {
		 FunctionIterator allFuncs = currentProgram.getFunctionManager().getFunctionsNoStubs(false);
		 ...
		 for(Function f : allFuncs) {
			 ...
			 if(!String.valueOf(ctg).contains("SmmInstallProtocolInterface"))
				 continue;

			 println(String.valueOf(f) + " has SmmInstallProtocolInterface");

			 int i,j;
			 ClangNode res = null;
			 for(i=0; i<ctg.numChildren(); i++) {
				 ClangNode cn = ctg.Child(i);
				 if(String.valueOf(cn).contains("SmmInstallProtocolInterface")) {
			 ...
			 if(res!=null) {
				 ...
				 String[] arr = inst.split("SmmInstallProtocolInterface")[1].split(",");
				 // i=0       =>  handle
				 // i=1       =>  guid
				 // i=2       =>  type
				 // i=3       =>  interface
				 ...
				 if(sympis.contains(")")) { // remove ");"
					 sympis = sympis.split("\\)")[0];
				 }
				 println("protocol interface symbol: " + sympis);

				 Symbol sym = currentProgram.getSymbolTable().getGlobalSymbols(sympis).get(0);
				 Address start = sym.getAddress();
				 int data = ByteBuffer.wrap(getBytes(start, 4)).order(ByteOrder.LITTLE_ENDIAN).getInt();
				 while(getDataAt(start)!=null && data!=0) {
